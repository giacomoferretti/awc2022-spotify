...
const PlaylistHeader = ({
  ...
  const isSaved = isSavedPlaylist(currentUser!.username, playlist.id);
  ...
  const savePlaylist = () => {
    addSavedPlaylist(currentUser!.username, playlist.id);
  };

  const removePlaylist = () => {
    removeSavedPlaylist(currentUser!.username, playlist.id);
  };
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-end">
          ...
          {!isOwner && isSaved && (
            <button
              ...
              title="Rimuovi playlist"
              className="mb-2 flex items-center gap-2 text-sm text-green-400 hover:underline">
...
const ShowPlaylist = () => {
  ...
  }, [getTrackById, playlist]);

  const owner = useMemo(
    () => getUserByUsername(playlist!.owner),
    [getUserByUsername, playlist]
  );

  const isOwner = useMemo(
    () => getCurrentUser()?.username === playlist!.owner,
    [getCurrentUser, playlist]
  );
